...
const Results: React.FC<ResultsProps> = ({ problems, userCodes, evaluations, onRetry }) => {
  ...
  // 복사 붙여넣기 의심 코드 분석
  const copyPasteAnalysis = userCodes.map((code, index) => ({
    problemIndex: index,
  ...
  const suspiciousProblems = copyPasteAnalysis.filter(item => item.detection.isCopyPaste);
    ...
    <div className="p-8">
        ...
        <div className="mb-8 p-6 bg-gradient-to-r from-red-100 to-orange-100 border-2 border-red-400 rounded-lg shadow-lg">
          ...
          <div className="mt-4 p-3 bg-red-200 rounded-lg">
            <p className="text-red-800 font-semibold text-center">
              ⚠️ 이 결과는 복사 붙여넣기 의심으로 인해 재검토가 필요할 수 있습니다.
            </p>
          </div>
        </div>
      )}
      ...
      <div className="space-y-4 mt-8">
        ...
        {problems.map((problem, index) => {
          const copyPasteInfo = copyPasteAnalysis[index];
          const isSuspicious = copyPasteInfo.detection.isCopyPaste;
          const isHighRisk = copyPasteInfo.detection.confidence >= 80;
            ...
            <div key={index} className={`p-4 rounded-lg border-2 ${
              ...
              <div className="flex items-center justify-between mb-2">
                ...
                {isSuspicious && (
                  <span className={`px-2 py-1 text-xs font-bold rounded-full ${
                    ...
                    {isHighRisk ? '🚨 높은 위험' : '⚠️ 의심'} ({copyPasteInfo.detection.confidence}%)
                  </span>
                )}
              </div>
              ...
              {isSuspicious && (
                <div className={`mt-2 p-2 rounded text-xs ${
                  ...
                  <strong>복사 붙여넣기 의심:</strong> {copyPasteInfo.detection.reasons.slice(0, 2).join(', ')}
                  {copyPasteInfo.detection.reasons.length > 2 && (
                    <span className="text-gray-600">...외 {copyPasteInfo.detection.reasons.length - 2}개</span>
                  )}
                </div>
              )}
